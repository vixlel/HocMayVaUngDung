...
upper = np.vstack([x1_dec, -(w[0] * x1_dec + b - 1) / w[1]]).T
lower = np.vstack([x1_dec, -(w[0] * x1_dec + b + 1) / w[1]]).T
...
plt.show()
plt.savefig('soft_margin_plot.png')
with open('svm_model.pkl', 'wb') as file:
    pickle.dump({'w': w, 'b': b, 'slack': slack}, file)